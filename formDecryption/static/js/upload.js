...
const file_actions = {
    ...
    upload: function (event) {
        // страничку не обновляем, это в данном контексте необходимо, поскольку
        // эта функция привязывается к форме в качестве callback-а (функция обратного вызова),
        // запускается при отправке формы на сервер, по дефолту отправка формы обновляет страничку, а это препятствует нормальной загрузке файлов.
...
function modal_window(data, add_class){
    ...
    $('[data-action="close_modal_error"]').one('click', function (){
        $('[data-action="modal_error_window"]').animate({'opacity': 0, 'z-index': '-2'}, 600, null, function (){
...
$(function() {
    ...
    $('[data-action="send_photos"]').on('click', file_actions.send_files)
})